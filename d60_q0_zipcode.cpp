...
void correctZipAndSortLetters(vector<ZipInfo>& zipinfo, vector<Letter>& letters) {
 //**Begin Insert**
    ...
    int n = zipinfo.size();
    for(int i = 0;i<n;i++){
        mp[make_pair(zipinfo[i].district,zipinfo[i].province)] = zipinfo[i].zip;
    }
    n = letters.size();
    for(int i = 0;i<n;i++){
        if(mp.count(make_pair(letters[i].district,letters[i].province))){
            letters[i].zip = mp[make_pair(letters[i].district,letters[i].province)];
        }
    }
